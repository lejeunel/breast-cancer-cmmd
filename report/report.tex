...
\date{\today}
\title{Breast Cancer Screening from Mammograms\\\medskip
\large Investigating Multi-View and Multi-Task Approaches}
\hypersetup{
 pdfauthor={Laurent Lejeune},
 pdftitle={Breast Cancer Screening from Mammograms},
 pdfkeywords={},
 pdfsubject={},
...
\label{sec:org19e3e7b}

\subsection{\label{screening}Screening}
\label{sec:org24c282c}
...
be addressed with special care \autocite{song22}.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=8cm]{./images/previews.png}
\caption{\label{fig:preview}Example images. On each row, we show two views of the same breast. For each image, we give its tumor type and the set of abnormalities that it contains.}
\end{figure}
\subsection{\label{split}Curation and Splitting}
\label{sec:org1615093}
