...
1. [[*Makefile/Docker Interface]]: A Makefile is provided that defines all pipeline steps and
  their dependencies. Under the hood, we leverage a docker container to execute commands.
2. [[*Python CLI]]: This requires that the user must install the provided package
